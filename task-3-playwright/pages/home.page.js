...
export class HomePage extends BasePage {
	...
	async getCardId(cardNumber = 1) {
		if (cardNumber < 1 || cardNumber > 9) {
			throw new Error(`Card number must be between 1 and 9, got: ${cardNumber}`);
		}

		const card = await this.productCards.nth(cardNumber - 1).getAttribute("data-test");
	...
	async goToNextPage() {
		...
		await this.page.locator(`[data-test="${firstCardId}"]`).waitFor({ state: "detached" }, 10000);
	}
}
